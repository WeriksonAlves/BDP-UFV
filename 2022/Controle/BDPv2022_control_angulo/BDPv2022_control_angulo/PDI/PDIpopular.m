...
if ~isempty(PDI.CoresMM)
    if ~isempty(PDI.CoresMM{2})
        ...
        Bola.X = PDI.CoresMM{2}(Bola.maxArea,1:2);
         Bola2.ang = atan2((Bola.X(2)-0),(Bola.X(1)-750));

% tratamento de borda
if abs(JogBDP(1).pPos.X(1,1)) > 670 || abs(JogBDP(1).pPos.X(2,1)) > 550
    if  abs(Bola2.ang) > pi/2
        Bola2.ang = -pi;
    else
        Bola2.ang = 0;
end
...
Bola2.X = [150*cos(Bola2.ang)+Bola.X(1) 150*sin(Bola2.ang)+Bola.X(2)];
