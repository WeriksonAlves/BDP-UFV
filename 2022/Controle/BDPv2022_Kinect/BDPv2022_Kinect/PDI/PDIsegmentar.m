...
% PDI.Imagem.RGB = Cam.Video.snapshot;
PDI.Imagem.RGB = getsnapshot(Cam.Video);


% Redimensionar imagem
PDI.Imagem.RGB = PDI.Imagem.RGB(min(Campo.Real.Recorte(:,2)):max(Campo.Real.Recorte(:,2)),...
    min(Campo.Real.Recorte(:,1)):max(Campo.Real.Recorte(:,1)),:);

% Correção de espalhamento da imagem
PDI.Imagem.RGB = flipud(PDI.Imagem.RGB);
