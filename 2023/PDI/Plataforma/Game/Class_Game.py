...
class GameWindow(object):
    ...
    def Create_Button(self):
        ...
        But_ViewSegmentation = Button(self.Window, text="Ver Segmentação", command = lambda: threading.Thread(target=self.Command_ViewSegmentation).start())
    ...
    def Command_SaveSettings(self): # Voltar aqui depois
        ...
        Player_Information = np.array([[self.Var_P1_Function.current(), self.Var_P1_Color.current()],
        ...
        if ((-1 in Matrix)==False) and ((self.Var_Cyan_Team.get()==True) or (self.Var_Yellow_Team.get()==True)) and ((self.Var_Left_Attack.get()==True) or (self.Var_Right_Attack.get()==True)):
            # self.Game_StatusBar_Clear()
            # self.Game_StatusBar_Set("Salvando calibração")
            # self.Var_SaveSettings = Matrix
            # print(Matrix)
            # np.savetxt('ConfiguraçõesJogadores.txt',Matrix)
    ...
    def Command_StartComunication(self):
        start = time.time()
        try:
            self.porta = serial.Serial('COM3', 115200, timeout=2)
    ...
    def Command_StartGame(self):
        ...
        while self.Var_Play == True:
            ...
            if(len(self.Posture_P1) != 0): self.Var_Game_Parameters[0, 0:] = self.Posture_P1
            if(len(self.Posture_P2) != 0): self.Var_Game_Parameters[1, 0:] = self.Posture_P2
            if(len(self.Posture_P3) != 0): self.Var_Game_Parameters[2, 0:] = self.Posture_P3
            if(len(self.Position_Oponent_1) != 0): self.Var_Game_Parameters[3, 0:2] = self.Position_Oponent_1
            if(len(self.Position_Oponent_2) != 0): self.Var_Game_Parameters[4, 0:2] = self.Position_Oponent_2
            if(len(self.Position_Oponent_3) != 0): self.Var_Game_Parameters[5, 0:2] = self.Position_Oponent_3
    ...
    #;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    def Command_SearchBall(self, frame, vector_limites):
    ...
    #;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    def Command_SearchPositionColor(self, frame, vector_limites):
        #encontra as posições de todos os objetos de cor pré-determinada na imagem segmentada.
        MinArea = 150
        MaxArea = 300
        PositionColor = []
    ...
    #;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    def Command_SearchPositionColor_Opp(self, frame, vector_limites):
        #encontra as posições de todos os objetos de cor pré-determinada na imagem segmentada.
        MinArea = 150
        MaxArea = 300
        PositionColor_Opp = []
    ...
    #;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    def Command_Associate_Single_Shirts(self, Position_Shirt_Team, Position_Shirts_Players):
    ...
    def Command_ViewAssociation(self):
        while True:
            ...
            cv2.imshow("Visão Associação", Campo_Virtual)
            cv2.waitKey(self.Var_FPS) #Está em 25 milisegundos = 40 fps
            if (cv2.getWindowProperty("Visão Associação", cv2.WND_PROP_VISIBLE) <1):
                break
    #;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    def Game_Action(self):
        P1 = MY_TEAM(self.Var_MyTeam_Color_BGR,self.Color_Player_1,self.Var_AttackSide,self.Var_P1_Function)
        P2 = MY_TEAM(self.Var_MyTeam_Color_BGR,self.Color_Player_2,self.Var_AttackSide,self.Var_P2_Function)
        P3 = MY_TEAM(self.Var_MyTeam_Color_BGR,self.Color_Player_3,self.Var_AttackSide,self.Var_P3_Function)

        P1.rBDP_pPos_X[0:, 0] = self.Var_Game_Parameters[0, 0:]
        P1.rBDP_pPos_Xd[0:, 0] = self.Var_Game_Parameters[6, 0:]
        P1.xtil()
        P1.autonivel()
        P1.baixonivel()
        ...
        # print(P1.rBDP_pSC_PWM[0,0], P1.rBDP_pSC_PWM[1,0])
        # print(P2.rBDP_pSC_PWM[0,0], P2.rBDP_pSC_PWM[1,0])
