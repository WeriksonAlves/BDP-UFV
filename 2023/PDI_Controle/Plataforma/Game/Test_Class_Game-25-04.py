...
from Game.MechanicalTest import*
...
class GameWindow(object):
        ...
        try:
            ...
            if self.Var_Comunication == False:
                ...
                while True:
                    ...
                    print(n,t)
                    self.pArduino.write([1, 2, int(150+n), int(150+n), int(150+n), int(150+n), 150, 150, 3, 10])
    ...
    #;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    def Command_MechanicalTest(self,Rx=300,Ry=200,T = 50):
        P1 = MY_TEAM(self.Var_MyTeam_Color_BGR,self.Color_Player_1,self.Var_AttackSide,self.Var_P1_Function.current())
        P2 = MY_TEAM(self.Var_MyTeam_Color_BGR,self.Color_Player_2,self.Var_AttackSide,self.Var_P2_Function.current())
        P3 = MY_TEAM(self.Var_MyTeam_Color_BGR,self.Color_Player_3,self.Var_AttackSide,self.Var_P3_Function.current())

        # Temporização
        tsim = T   # Tempo total da simulação
        tap = 0.02  # taxa de atualização do Jogador
        ...
        w = (2*np.pi/T) #Frequência em rad/s        
        Rx = Rx
        Ry = Ry
        ...
        self.Clear_StatusBar()
        self.Set_StatusBar("Elipse Iniciada")

        # Simulação em tempo de máquina
        # for t in np.linspace(0,tsim,int(tsim/tap)):
        t0 = time.time()
        
        while (time.time() - t0) < tsim:
            t = time.time() - t0
            # Data aquisition
            # P.rGetSensorData
            ...
            # Trajetoria Eliptica:
            P1.rBDP_pPos_Xd[0:, 0] = [Rx*np.cos(w*t)-375, Ry*np.sin(w*t),0]
            P1.rBDP_pPos_dXd[0:, 0] = [-Rx*w*np.sin(w*t), Ry*w*np.cos(w*t),0]
            P1.xtil()
            P1.autonivel()
            P1.baixonivel()

            # Trajetoria Eliptica:
            P2.rBDP_pPos_Xd[0:, 0] = [0,0,0]
            P2.rBDP_pPos_dXd[0:, 0] = [-Rx*w*np.sin(w*t), Ry*w*np.cos(w*t),0]
            P2.xtil()
            P2.autonivel()
            P2.baixonivel()

            # Trajetoria Eliptica:
            # P3.rBDP_pPos_Xd[0:, 0] = [Rx*np.cos(w*t), Ry*np.sin(w*t),0]
            P3.rBDP_pPos_Xd[0:, 0] = [0,0,0]
            P3.rBDP_pPos_dXd[0:, 0] = [-Rx*w*np.sin(w*t), Ry*w*np.cos(w*t),0]
            P3.xtil()
            P3.autonivel()
            P3.baixonivel()
        ...
        self.Clear_StatusBar()
        self.Set_StatusBar('Elipse Encerrada')
    #;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
